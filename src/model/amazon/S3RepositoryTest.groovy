...
//		Repository brepo = (Repository)obj;
		...
		repo.setPath("/WEB-INF/data/test/originals/sub1");
	...
	public void testPutAPI() throws Exception
	{
		...
		Page testfile = archive.getPageManager().getPage("/WEB-INF/data/test/originals/sub1/sub2/server.png");
		S3Repository repo = getRepo();

		List names = repo.getChildrenNames("/");
		assertTrue(names.size() > 0);

		repo.put(testfile.getContentItem());

		ContentItem item = repo.getStub(names.get(0));
		assertNotNull( item );

		ContentItem i = repo.get("/WEB-INF/data/test/originals/sub1/sub2/server.png");
		assertNotNull(i);
